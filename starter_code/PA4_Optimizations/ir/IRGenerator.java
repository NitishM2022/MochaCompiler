...
public class IRGenerator implements NodeVisitor {
    ...
    private Value loadIfNeeded(Value val) {
        if (val instanceof Variable && !((Variable) val).isTemp()) {
            ...
            addInstruction(new Load(nextInstructionId(), temp, val));
            return temp;
        }
        return val;
    }
    ...
    private void initializeVariableToDefault(Variable var) {
        ...
        addInstruction(new Store(nextInstructionId(), defaultValue, var));
    }
    ...
    public void visit(Assignment node) {
        if (node.getSource() != null) {
            ...
            if (node.getDestination() instanceof Designator) {
                ...
                addInstruction(new Store(nextInstructionId(), srcValue, new Variable(sym)));
    ...
    private void handleIncrementDecrement(Expression dest, Token op) {
        ...
        if (dest instanceof Designator) {
            ...
            Variable temp = getTemp();
            addInstruction(new Load(nextInstructionId(), temp, destVar));
            ...
            addInstruction(new Store(nextInstructionId(), result, destVar));
    ...
    public void visit(IfStatement node) {
        ...
        if (elseBlock != null) currentCFG.addBlock(elseBlock);
    ...
    public void visit(FunctionDeclaration node) {
        // Save the current initialized variables set and create a new one for this function
        Set<String> savedInitializedVars = initializedVariables;
        initializedVariables = new HashSet<>();
        
        symbolTable.enterScope();
        
        // Add function parameters to initialized variables (they're always initialized by caller)
