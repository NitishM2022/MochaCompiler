...
public class Optimizer {
    ...
    public String applyOptimizations(List<String> opts, List<CFG> cfgs, boolean loop, boolean max) {
        ...
        boolean applyOrphanElimination = false;
        
        if (max) {
            optimizationsToApply = Arrays.asList("cf", "cp", "dce", "cse");
        } else if (opts != null && !opts.isEmpty()) {
            optimizationsToApply = new ArrayList<>();
            for (String opt : opts) {
                if (opt.equalsIgnoreCase("orphan") || opt.equalsIgnoreCase("ofe")) {
                    applyOrphanElimination = true;
                } else if (opt.equalsIgnoreCase("cpp")) {
                    if (!optimizationsToApply.contains("cp")) {
                        optimizationsToApply.add("cp"); // cpp is an alias for cp
                    }
                } else {
    ...
    private boolean optimizeCFG(CFG cfg, String optName) {
        switch (optName.toLowerCase()) {
            ...
            case "cp":
                // "cp" and "cpp" are the same pass
                return new CopyAndConstantPropagation(this).optimize(cfg);
            case "dce":
