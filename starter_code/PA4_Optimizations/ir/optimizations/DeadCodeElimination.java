...
public class DeadCodeElimination extends BaseOptimization {
    ...
    public boolean optimize(CFG cfg) {
        ...
        while (!worklist.isEmpty()) {
            ...
            if (deadInst.isEliminated() || hasSideEffects(deadInst)) {
                continue;
            }
            ...
            for (Value operand : operands) {
                if (!(operand instanceof Variable)) continue;
                ...
                if (users != null) {
                    ...
                    if (users.isEmpty() && !hasSideEffects(def) && !worklist.contains(def)) {
                        worklist.add(def);
                    }
                }
            }
    ...
    private List<Value> getOperands(TAC instruction) {
        if (instruction instanceof Phi) {
            ...
            return phi.getArgs() != null ? 
                   new ArrayList<>(phi.getArgs().values()) : 
                   Collections.emptyList();
        }
        return instruction.getOperands() != null ? 
