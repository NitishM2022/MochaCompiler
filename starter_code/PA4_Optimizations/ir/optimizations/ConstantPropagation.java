...
public class ConstantPropagation extends BaseOptimization {
    ...
    public ConstantPropagation(Optimizer optimizer) {
        super(optimizer);
    }
        ...
        for (Value arg : args.values()) {
            ...
            if (result == null) {
                ...
                if (result.type == LatticeType.CONSTANT) {
                    if (!constantEquals(result.value, argValue.value))
                        return LatticeValue.BOTTOM();
                }
            }
        }
    ...
    private LatticeValue evaluateMov(Mov mov, Map<Variable, LatticeValue> lattice) {
        ...
        // Propagate constant from source variable
            ...
            for (Phi phi : block.getPhis()) {
                ...
                if (args != null) {
                    ...
                    if (phiChanged) {
                        phi.setArgs(newArgs);
                        changed = true;
                        logInstruction(phi, "Constant propagated in phi: " + phi.toString());
                    }
                }
            }
            ...
            for (TAC instruction : block.getInstructions()) {
                ...
                if (operands != null && !operands.isEmpty()) {
                    ...
                    if (instChanged) {
                        ...
                        changed = true;
                        logInstruction(instruction, "Constant propagated in: " + instruction.toString());
                    }
                }
            }
        }
    ...
    private Value getReplacement(Value value, Map<Variable, LatticeValue> lattice) {
        ...
        if (lv != null && lv.type == LatticeType.CONSTANT) {
            return lv.value;
        }
